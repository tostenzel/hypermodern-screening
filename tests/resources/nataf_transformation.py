...
class nataf_transformation(object):  # pragma: no cover
    ...
    def X2U(self, X, Jacobian=False):
        ...
        if np.shape(self.Marginals)[0] != np.shape(X)[0]:
            X = X.T
        m = X.shape[0]
        Z = np.zeros(X.shape)
        if np.size(self.Marginals) == 1:
            ...
            for i in range(m):
                Z[i, :] = stats.norm.ppf(self.Marginals[i].cdf(X[i, :]))
            diag = np.zeros([m, m])
        U = np.linalg.solve(self.A, Z.squeeze())
        if not Jacobian:
            return U
        else:
            for i in range(m):
    ...
    def U2X(self, U, Jacobian=False):
        ...
        if np.shape(self.Marginals)[0] != U.shape[0]:
            U = U.T
        Z = np.dot(self.A, U)
        m, n = U.shape
        X = np.zeros(U.shape)
    ...
    def jointpdf(self, x):
        ...
        phi_n = stats.multivariate_normal.pdf(U.T, mu, self.Rho_Z)
        # print(phi_n)
        jointpdf = np.zeros(n)
        for i in range(n):
            try:
                jointpdf[i] = (np.prod(f[:, i]) / np.prod(phi[:, i])) * phi_n[i]
    ...
    def jointcdf(self, x):
        ...
        mu = np.zeros([1, s])
        for i in range(s):
            U[:, i] = stats.norm.ppf(self.Marginals[i].cdf(x[:, i]), loc=0, scale=1)
